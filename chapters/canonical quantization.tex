...
\begin{itemize}
	...
	\begin{tcolorbox}[title=calculation:]
		\begin{align}
			...
			=& \int \frac{d^D p}{\rho(p)} \frac{d^D q}{\rho(q)} \Big( \Big( (\omega_p a_{\vec{q}} a^\dag_{\vec{p}} + \omega_q a^\dag_{\vec{p}} a_{\vec{q}}) e^{i (\omega_p - \omega_q) \cdot t} - (\omega_p b^\dag_{\vec{q}} b_{\vec{p}} + \omega_q b_{\vec{p}} b^\dag_{\vec{q}}) e^{- i (\omega_p - \omega_q) \cdot t} \Big) (2 \pi)^D \delta^{(D)}(\vec{p} - \vec{q}) \notag \\
