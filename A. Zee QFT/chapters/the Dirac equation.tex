...
\begin{itemize}
	...
	\item Dirac 方程的 Lorentz covariance 见 \eqref{the Dirac spinor.2.10}.
\end{itemize}

\section{Dirac Lagrangian}
\begin{itemize}
	\item 根据 \eqref{the Dirac spinor.2.19} 以及之前标量场的计算经验, 可知
	\begin{equation} \label{the Dirac equation.2.1}
...
\subsection{internal vector symmetry} \label{subsection the Dirac equation.3.1}
\begin{itemize}
	\item 做变换 $\Psi \mapsto e^{i \theta} \Psi$, Lagrangian 保持不变, 利用 Noether's theorem 得到守恒流 (见 section \ref{classical field theory and Noether's theorem.2}),
	\begin{equation} \label{the Dirac equation.3.3}
	...
	\begin{tcolorbox}[title=calculation:]
		计算广义动量,
		\begin{equation} \label{the Dirac equation.3.4}
			\begin{dcases}
		...
		这里看起来有点奇怪 (canonical transformation), 需要再说明一下. 对于 \eqref{the Dirac equation.2.1} 第一个等号后边,
		\begin{equation}
			\begin{dcases}
			...
			\end{dcases},
		\end{equation}
		对于 \eqref{the Dirac equation.2.1} 第二个等号后边, 忽略掉全微分项,
		\begin{equation}
			\begin{dcases}
...
\begin{itemize}
	...
	\begin{tcolorbox}[title=calculation:]
		...
		\end{equation}
		其中用到了 $\gamma^0 \gamma^\mu \gamma^0 = \gamma^{\mu \dag}$, 见 \eqref{the Dirac spinor.2.15}.
	\end{tcolorbox}
	
	\item $\Psi_c = \gamma^2 \Psi^*$ 满足如下方程 (对比 \eqref{the Dirac equation.6.1}),
	\begin{equation}
...
\begin{itemize}
	...
	\end{equation}
	其中 $K$ 是 complex conjugation operator (见 appendix \ref{antiunitary operator and time reversal}). 另外, 有 $T^2 = - 1$, 符合预期.
...
\begin{itemize}
	...
	\begin{equation} \label{the Dirac equation.6.1}
	...
	\end{equation}
	可见电子和电磁场耦合项为 $- e A_\mu J_V^\mu$, 其中 $J_V^\mu$ 是 internal vector symmetry 的守恒流, 见 \eqref{the Dirac equation.3.3}.
