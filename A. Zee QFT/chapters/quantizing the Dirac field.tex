...
\section{anticommutation} \label{quantizing the Dirac field.1}
\begin{itemize}
...
\begin{itemize}
	...
	\begin{equation} \label{quantizing the Dirac field.3.1}
		\begin{dcases}
	...
	\item 回顾 section \ref{canonical quantization.4} 关于 complex scalar field 的内容, 可知 $b^\dag$ 和 $c^\dag$ 产生的粒子具有相反的电荷, 不妨令 $b^\dag$ 产生 electron (带电荷 $- e$), $c^\dag$ 产生 position (带电荷 $e$).
	
	\item section \ref{quantizing the Dirac field.1} 中的讨论说明
	\begin{equation} \label{quantizing the Dirac field.3.2}
		\begin{dcases}
	...
	\item $\Psi$ 的 momentum conjecture 为 ($\pi_\Psi^\mu$ 见 \eqref{the Dirac equation.3.4})
	\begin{equation}
	...
	\begin{tcolorbox}[title=calculation:]
		代入 \eqref{quantizing the Dirac field.3.2}, (下式中 $x = (t, \vec{x}), y = (t, \vec{y})$, 另外注意到 $u \bar{u} = u u^\dag \gamma^0$),
		\begin{align}
...
\begin{itemize}
	...
	\end{equation}
	其中 vacuum energy,
	\begin{equation} \label{quantizing the Dirac field.4.2}
...
\begin{itemize}
	...
	\begin{tcolorbox}[title=calculation:]
		energy-momentum tensor 的 $0, \mu$ 分量为 (见 \eqref{the Dirac equation.4.1})
		\begin{align}
...
\begin{itemize}
	...
	\end{align}
	其中, $M^{\mu \nu \rho}$ 见 \eqref{the Dirac equation.4.2}.
	\begin{itemize}
...
\begin{itemize}
	\item internal vector symmetry 对应的守恒流就是电流, 见 subsection \ref{subsection the Dirac equation.3.1}, 有
	\begin{equation}
...
\begin{itemize}
	\item 参考 scalar field 中的 propagator (见 \eqref{canonical quantization.1.17}), the propagator of the Dirac field is
	\begin{align}
		...
		&= \int \frac{d^4 p}{(2 \pi)^4} \frac{i}{\cancel{p} - m + i \epsilon} e^{- i p \cdot (x - y)}, \label{quantizing the Dirac field.6.1}
	\end{align}
	其中
	\begin{equation}
