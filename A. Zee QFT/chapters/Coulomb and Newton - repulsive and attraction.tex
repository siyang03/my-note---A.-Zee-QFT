...
\begin{itemize}
	...
	\begin{equation} \label{Coulomb and Newton: repulsive and attraction.1.1}
	...
	考虑到积分需要收敛, 作替换 $m^2 \mapsto m^2 - i \epsilon$, (为什么 $A_\mu$ 类空, 只知道 $\tilde{A}_\mu$ 类空, 见 subsection \ref{subsection Coulomb and Newton: repulsive and attraction.1.2}, 但路径积分中的 $A$ 显然不满足 field equation $\Longrightarrow$ 路径积分中起主要作用的 $\tilde{A}$ 类空, 因此 $- \epsilon |\tilde{A}|^2 < 0$).
...
\subsection{spin \& polarization vector} \label{subsection Coulomb and Newton: repulsive and attraction.1.1}
\begin{itemize}
...
\subsection{Maxwell Lagrangian} \label{subsection Coulomb and Newton: repulsive and attraction.1.2}
\begin{itemize}
	\item 根据 \eqref{Coulomb and Newton: repulsive and attraction.1.1} 中的 Lagrangian density, 得到 field equation,
	\begin{equation} \label{Coulomb and Newton: repulsive and attraction.1.11}
		\Big( - \partial^\mu \partial^\nu + \eta^{\mu \nu} (\partial^2 - m^2) \Big) A_\nu = 0.
	\end{equation}
	\begin{itemize}
		\item spin-$1$ particle 有 3 个自旋自由度, 而 $A_\mu$ 有 4 个分量, 所以需要一个约束方程 (只在 $m \neq 0$ 情况下存在),
		\begin{equation} \label{Coulomb and Newton: repulsive and attraction.1.12}
			\partial^\mu A_\mu = 0 \iff k^\mu \tilde{A}_\mu(k) = 0.
		\end{equation}
		来源: 在 \eqref{Coulomb and Newton: repulsive and attraction.1.11} 左右两边作用一个 $\partial_\mu$ 即可得到这个约束方程.
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item 受 subsection \ref{subsection Coulomb and Newton: repulsive and attraction.1.1} 启发, 对于 spin-$2$ particle, 其极化矢量有 5 个方向, 满足
	\begin{equation} \label{Coulomb and Newton: repulsive and attraction.2.1}
		\begin{dcases}
			k^\mu (\epsilon^a)_{(\mu \nu)} = 0 \\
			\eta^{\mu \nu} (\epsilon^a)_{(\mu \nu)} = 0
		\end{dcases},
	...
	\begin{itemize}
		...
		\item 与 subsection \ref{subsection Coulomb and Newton: repulsive and attraction.1.1} 中提示一样, 粒子的极化方向用 $e^a$ 表示.
	\end{itemize}
	...
	\begin{tcolorbox}[title=calculation:]
		...
		\end{align}
		代入 \eqref{Coulomb and Newton: repulsive and attraction.2.1} 得
		\begin{equation}
			\begin{dcases}
				0 = - A + B + 2 C = - B + D = - C + E \\
...
\section{remarks}
\begin{itemize}
	\item 由于 seesaw mechanism (见 subsection \ref{perturbation theory in QM.1.1}), 引入扰动一般会降低基态能量, 因此大多数相互作用表现为吸引, 而 spin-$1$ 表现为同性相斥是因为 $\eta^{0 0} = - 1$.
